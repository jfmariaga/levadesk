...
class Index extends Component
{
    ...
    //     // Tickets donde el usuario es agente asignado
    //     $query = Ticket::where('asignado_a', $userId)
    //         // O donde el usuario tiene tareas activas y es colaborador
    //         ->orWhereHas('tareas', function ($t) use ($userId) {
    ...
    //     // Cargar relaciones y solo tareas activas del usuario
    //     $tickets = $query->with([
    //         'urgencia',
    //         'estado',
    //         'colaboradores',
    ...
    //         if ($ticket->asignado_a == $userId) {
    //             $ticket->rol = 'Agente';
    //         } elseif ($ticket->tareas->where('user_id', $userId)->isNotEmpty()) {
    //             $ticket->rol = 'Colaborador';
    //         } else {
    //             $ticket->rol = null; // no debería mostrarse
    //         }
    //         return $ticket;
    //     })->filter(fn($t) => $t->rol !== null); // elimina los sin rol (sin tareas activas)

    //     // Estadísticas
    //     $this->ticketsSolucionados = $this->tickets->where('estado_id', 4);
    //     $this->ticketsEnProceso = $this->tickets->whereIn('estado_id', [
    //         3,
    //         6,
    //         7,
    //         8,
    ...
    public function cargarDatos()
    {
        ...
        $query = Ticket::where(function ($q) use ($userId) {
        ...
        // ✅ Ahora todos estos filtros se aplicarán correctamente a ambos casos:
        if (is_array($this->SelectedEstado) && !empty($this->SelectedEstado)) {
            $query->whereIn('estado_id', $this->SelectedEstado);
        }
        ...
        // Carga relaciones y tareas activas
        $tickets = $query->with([
            'urgencia',
            'estado',
            'colaboradores',
